...
export function SkillsWithProjects() {
  ...
  return (
    <div className="space-y-8">
          ...
          >
            ...
            >
              <span className="relative z-10 flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${skill.color}`} style={{ opacity: skill.level / 100 }} />
                {skill.name}
              ...
              <div
                ...
                style={{ width: selectedSkill === skill.name ? "100%" : `${skill.level}%` }}
              />
            </Badge>
          </motion.div>
        ))}
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`w-4 h-4 rounded-full ${getSkillData(selectedSkill)?.color}`} />
                    <CardTitle className="text-2xl text-white">{selectedSkill}</CardTitle>
                    <Badge variant="outline" className="text-purple-400 border-purple-400">
                      {getSkillData(selectedSkill)?.level}% Proficiency
                    </Badge>
                  </div>
                  <Button
                    variant="ghost"
                ...
                <CardDescription className="text-zinc-400">
                  Projects utilizing {selectedSkill} ({getSkillProjects(selectedSkill).length} projects)
                </CardDescription>
              </CardHeader>
              <CardContent>
      ...
      >
        <p className="text-zinc-500 text-sm">Click on any skill badge to see the projects where it was applied</p>
      </motion.div>
    </div>
  )
}
